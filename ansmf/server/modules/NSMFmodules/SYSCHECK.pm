...
sub import {
    ...
    # Disconnect        
    $dbh->disconnect;
    print "[**] SYSCHECK: Disconnected to DB...\n";
    $dbh = undef; # undef it, or else there will be a threading issue...
}

sub DESTROY {
    print "[**] SYSCHECK: Dead on arrival!\n";
}
...
sub SYSCHECK {
    ...
    $DEBUG = $REQ->{'debug'};
    $NODENAME = $REQ->{'node'};
    print "[**] SYSCHECK: Huston - we got a connection from $NODENAME.\n" if $DEBUG;
    if (not defined $dbh) {
        print "[**] SYSCHECK: Connecting to database...\n";
        $dbh = DBI->connect($DBI,$DB_USERNAME,$DB_PASSWORD, {RaiseError => 1}) or die "$DBI::errstr";
        print "[**] SYSCHECK: Connection OK...\n";
    ...
    while ($RS) {
        ...
        if ($line =~ /^(POST|GET|PING|BYE)/) {
            if ($line =~ /^PING (.*)/) {
                ...
                print "[**] SYSCHECK: Got POST DATA request...\n";
                got_post($REQ);
            }
            elsif ($line =~ /^GET/) {
                got_get($REQ);
                ...
                print "[**] SYSCHECK: Got BYE request...\n";
                got_bye($REQ);
                last;
            }
        }
...
sub got_bye {
    my $REQ = shift;
    my $RS = $REQ->{'requestsocket'};
    print "[**] SYSCHECK: shuting down socket...\n";
    $RS->shutdown(2);
}
...
sub got_post {
    ...
    if ( read_socket_data($REQ) == 1) {
        print $RS "200 OK ACCEPTED\0";
        $RS->flush();
        NSMFmodules::SYSCHECK::put_cxdata_to_db($REQ);
        print "[**] SYSCHECK: Data recieved OK from node: $node\n";
    } else {
        print $RS "204 DATA RECIEVE ERROR\0";
        $RS->flush();
        print "[**] SYSCHECK: Data recieve ERROR from node: $node\n";
    }
    print "[**] SYSCHECK: Leaving got_post()\n";
}
...
sub put_cxdata_to_db {
    ...
    foreach my $line (@lines) {
        ...
        $line =~ /^\d{19}/;
        unless($line) {
            print "[EE] SYSCHECK: Not valid session start format in: '$line'";
            next LINE;
        }
        my @elements = split /\|/,$line;
        unless(@elements == 15) {
            print "[EE] SYSCHECK: Not valid Nr. of session args format in: '$line'";
            next LINE;
        }
        # Things should be OK now to send to the DB
        print "[**] SYSCHECK: put_cxdata_to_db(): $line\n";
...
sub checkif_table_exist {
    ...
    if ($dbh->err) {
       print "[EE] SYSCHECK: Table $tablename does not exist.\n" if $DEBUG;
       return 0;
    }
    else{
       return 1;
...
sub put_session2db {
   ...
   if ($@ =~ /Duplicate entry/) { # Why dont this work?
      ...
      print "[**] SYSCHECK: Got duplicate entry....\n";
      return 0;
   } elsif ($@) {
        print "[EE] SYSCHECK: $@\n";
...
sub merge_session_tables {
   ...
   eval {
      ...
      print "[**] SYSCHECK: Creating session MERGE table\n" if $DEBUG;
      my $sql = "                                        \
   ...
   if ($@) {
      # Failed
      print "[EE] SYSCHECK: Create session MERGE table failed!\n" if $DEBUG;
      return 1;
   }
   return 0;
}
...
sub delete_merged_session_table {
    my ($sql, $sth);
    eval{
        $sql = "DROP TABLE IF EXISTS syscheck";
        $sth = $dbh->prepare($sql);
        $sth->execute;
        $sth->finish;
    };
    if ($@) {
        # Failed
        print "[**] SYSCHECK: Drop merge-table session failed...\n" if $DEBUG;
        return 1;
    }
    print "[**] SYSCHECK: Dropped merge-table...\n" if $DEBUG;
    return 0;
}
...
sub find_session_tables {
    my ($sql, $sth);
    my $tables = q();
    $sql = q(SHOW TABLES LIKE 'syscheck_%');
    $sth = $dbh->prepare($sql);
    $sth->execute;
...
END {
    ...
    warn "[**] SYSCHECK: Terminating module...\n";
